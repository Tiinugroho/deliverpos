...
@section('content')
    <div class="w-full space-y-4">
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <div>
                <h1 class="text-lg font-bold text-slate-900 tracking-tight">Form Tambah Kategori Baru</h1>
            </div>
        </div>

        <div class="glass-card bg-white rounded-3xl p-6 shadow-sm border border-slate-100 w-full">
            <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-5 text-xs text-slate-700">
                @csrf
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="category_name" value="{{ old('category_name') }}" required
                        ...
                        placeholder="Contoh: Makanan Berat, Cemilan, Minuman Dingin">
                    @error('category_name')
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="description" rows="4"
                        class="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-blue-500 focus:bg-white transition text-slate-800"
                        placeholder="Tuliskan keterangan pelengkap kelompok menu ini...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-[11px] text-rose-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
